...
// TestRegisterRoutesNoConflict mounts every engine nesting under /modules/:id on
// one router, verifying the combined route tree builds without a Gin panic.
func TestRegisterRoutesNoConflict(t *testing.T) {
	...
	NewModule(nil, noop, noop, noop, rbac.New(nil, nil), nil).RegisterRoutes(api)
}
